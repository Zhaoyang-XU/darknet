...
data random_go_moves(moves m, int n)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        if(row >= 19 || col >= 19){
            ...
            if(board[col + 19*row]) printf("");//printf("hey\n");
        }
...
void print_board(FILE *stream, float *board, int swap, int *indexes)
{
    ...
    for(j = 0; j < 19; ++j){
        ...
        for(i = 0; i < 19; ++i){
            ...
            if(indexes){
                ...
                for(n = 0; n < nind; ++n){
                    if(index == indexes[n]){
                         ...
                         */
                        if(n == 0) printf("");////fprintf(stream, " 1");
                        else if(n == 1) printf("");////fprintf(stream, " 2");
                        else if(n == 2) printf("");////fprintf(stream, " 3");
                        else if(n == 3) printf("");////fprintf(stream, " 4");
                        else if(n == 4) printf("");////fprintf(stream, " 5");
                    }
                }
                if(found) continue;
            }
            ...
            if(board[index]*-swap > 0) printf("");////fprintf(stream, " O");
            else if(board[index]*-swap < 0) printf("");////fprintf(stream, " X");
            else printf("");////fprintf(stream, "  ");
        }
        ////fprintf(stream, "\n");
    }
}
...
int print_game(float *board, FILE *fp)
{
    ...
    for(j = 0; j < 19; ++j){
        for(i = 0; i < 19; ++i){
            if(board[j*19 + i] == 1) printf("");////fprintf(fp, "play black %c%d\n", 'A'+i+(i>=8), 19-j);
            if(board[j*19 + i] == -1) printf("");////fprintf(fp, "play white %c%d\n", 'A'+i+(i>=8), 19-j);
...
void test_go(char *cfg, char *weights, int multi)
{
    ...
    while(1){
        ...
        if(color == 1) printf("");//printf("X Enter move: ");
        else printf("");//printf("O Enter move: ");
