...
static void print_cocos(FILE *fp, char *image_path, box *boxes, float **probs, int num_boxes, int classes, int w, int h)
{
    ...
    for(i = 0; i < num_boxes; ++i){
        ...
        for(j = 0; j < classes; ++j){
            if (probs[i][j]) printf("");////fprintf(fp, "{\"image_id\":%d, \"category_id\":%d, \"bbox\":[%f, %f, %f, %f], \"score\":%f},\n", image_id, coco_ids[j], bx, by, bw, bh, probs[i][j]);
        }
    }
}
...
void print_detector_detections(FILE **fps, char *id, box *boxes, float **probs, int total, int classes, int w, int h)
{
    ...
    for(i = 0; i < total; ++i){
        ...
        for(j = 0; j < classes; ++j){
            if (probs[i][j]) printf("");////fprintf(fps[j], "%s %f %f %f %f %f\n", id, probs[i][j],xmin, ymin, xmax, ymax);
        }
    }
}
...
void print_imagenet_detections(FILE *fp, int id, box *boxes, float **probs, int total, int classes, int w, int h)
{
    ...
    for(i = 0; i < total; ++i){
        ...
        for(j = 0; j < classes; ++j){
            ...
            int class1 = j;
            if (probs[i][class1]) printf("");////fprintf(fp, "%d %d %f %f %f %f %f\n", id, j+1, probs[i][class1],xmin, ymin, xmax, ymax);
        }
    }
}
...
#else
        _//snprintf(buff, 1024, "%s/%s.json", prefix, outfile);
#endif
        fp = fopen(buff, "w");
        ////fprintf(fp, "[\n");
        coco = 1;
...
#else
        _//snprintf(buff, 1024, "%s/%s.txt", prefix, outfile);
#endif
        fp = fopen(buff, "w");
        imagenet = 1;
        classes = 200;
...
#else
            _//snprintf(buff, 1024, "%s/%s%s.txt", prefix, outfile, names[j]);
#endif
            fps[j] = fopen(buff, "w");
        }
    }
